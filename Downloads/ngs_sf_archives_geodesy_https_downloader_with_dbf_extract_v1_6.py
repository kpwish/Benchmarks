#!/usr/bin/env python3
"""
NGS Archived ShapeFiles downloader (HTTPS, geodesy.noaa.gov) + DBF extraction

This version is designed around the confirmed-working endpoint:
    https://geodesy.noaa.gov/pub/DS_ARCHIVE/ShapeFiles/<STATE>.zip

Your curl test for AL.zip returned:
- HTTP 200
- Content-Type: application/zip
- A valid ZIP file

Behavior:
- Downloads per-state ZIPs from geodesy.noaa.gov using lowercase server filenames (<STATE>.zip).
- Saves locally with CAPITALIZED extension (<STATE>.ZIP) to match your prior convention.
- Validates downloads as real ZIPs (PK signature + zipfile check).
- Extracts any .dbf files from each ZIP into:
      ./ngs_sf_archive_downloads/dbfs/
  Prefixes extracted DBFs with the state code to avoid collisions.

Install:
    python -m pip install requests tqdm

Run:
    python ngs_sf_archives_geodesy_https_downloader_with_dbf_extract_v1_6.py
"""
...
def _download_zip_validated(session: requests.Session, url: str, out_path: str) -> None:
    ...
    if not _is_zip_magic(first_chunk):
        ...
        if _looks_like_html(first_chunk):
            raise RuntimeError(
                "Downloaded content is HTML, not a ZIP (likely an error page).\n"
                f"URL: {url}\n"
...
def main() -> None:
    ...
    session.headers.update(
        {
            "User-Agent": "ngs-shapefiles-geodesy-downloader/1.6",
            "Accept": "*/*",
    ...
    for code, name in STATE_CODES.items():
        server_name = f"{code}.zip"     # confirmed working
        local_name = f"{code}.ZIP"      # your preferred local naming
